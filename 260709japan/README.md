...
- 정답 화면의 `한자 복사하기` 버튼으로 출제된 한자를 바로 클립보드에 복사할 수 있습니다.
...
- soft lock 퀴즈에서는 오답을 2번 고르면 별도 안내 없이 새 문제로 넘어가 브루트포스 풀이를 막습니다.
- 앱에서 잠금 후 퀴즈를 켜면 Foreground Service가 잠금 해제 이벤트 뒤 퀴즈 화면을 띄웁니다.
- Android 14 이상에서는 잠금 해제 뒤 퀴즈가 바로 뜨도록 `전체 화면 알림` 허용이 필요할 수 있습니다. 앱에서 잠금 후 퀴즈를 켤 때 해당 설정 화면으로 안내합니다.
