...
- 한글 뜻을 4지선다 중에서 고릅니다.
- 매 문제에서 한자 단어를 먼저 보여주고, `3 → 2 → 1` 카운트다운 뒤에 선택지를 표시합니다.
...
- 앱에서 잠금 후 퀴즈를 켜면 Foreground Service가 잠금 해제 이벤트 뒤 퀴즈 오버레이를 띄웁니다.
- Android 설정에서 `다른 앱 위에 표시` 권한을 허용해야 잠금 해제 뒤 문제 화면이 다른 앱 위에 덮입니다.
...
Android는 일반 앱이 시스템 잠금화면을 완전히 대체하거나 강제로 막는 것을 허용하지 않습니다. 그래서 이 앱은 사용자가 켠 경우 Foreground Service로 잠금 해제 이벤트를 감지한 뒤 `TYPE_APPLICATION_OVERLAY` 퀴즈 화면을 띄우는 soft lock 구조입니다.
