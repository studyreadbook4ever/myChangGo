...
const dom = {
    canvas: query("#gpu-canvas"),
    noteLayer: query("#note-layer"),
    menu: query("#menu"),
    hud: query("#hud"),
    keyStrip: query("#key-strip"),
    startButton: query("#start-button"),
    status: query("#webgpu-status"),
    noteRate: query("#note-rate"),
    rateNumber: query("#rate-number"),
    rateOutput: query("#rate-output"),
...
function calculateTotalNotes(rate) {
...
function noteColorForKeyIndex(keyIndex) {
...
function render(now) {
    ...
    if (instanceCount > 0) {
        gpu.device.queue.writeBuffer(gpu.instanceBuffer, 0, instanceData.subarray(0, instanceCount * 8));
    }
...
function buildInstances(now) {
    instanceCount = 0;
    pushRect(layout.width / 2, layout.height / 2, layout.width, layout.height, [0.045, 0.048, 0.044, 1]);
    drawLanes();
    drawIdlePreview(now);
    drawNotes();
    drawEffects();
