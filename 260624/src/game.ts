...
const FIRST_SPAWN_AT = 0.35;
const SPAWN_CUTOFF = GAME_DURATION - NOTE_LIFETIME;
const SPAWN_EPSILON = 1e-9;
const MAX_SCORE = 1_000_000;
const HITTABLE_NOTE_LIMIT = 5;
const MAX_INSTANCES = 4096;

const KEY_INDEX = new Map<string, number>(KEY_ORDER.map((key, index) => [key, index]));
...
const dom = {
  canvas: query<HTMLCanvasElement>("#gpu-canvas"),
  noteLayer: query<HTMLDivElement>("#note-layer"),
  menu: query<HTMLElement>("#menu"),
  hud: query<HTMLElement>("#hud"),
  keyStrip: query<HTMLDivElement>("#key-strip"),
  startButton: query<HTMLButtonElement>("#start-button"),
  status: query<HTMLParagraphElement>("#webgpu-status"),
  noteRate: query<HTMLInputElement>("#note-rate"),
  rateNumber: query<HTMLInputElement>("#rate-number"),
  rateOutput: query<HTMLOutputElement>("#rate-output"),
  lastResult: query<HTMLDivElement>("#last-result"),
  score: query<HTMLElement>("#score"),
  combo: query<HTMLElement>("#combo"),
  timer: query<HTMLElement>("#timer"),
  accuracy: query<HTMLElement>("#accuracy"),
  feedback: query<HTMLDivElement>("#feedback"),
};
...
const keyCells = new Map<string, HTMLSpanElement>();
...
async function initWebGPU(): Promise<void> {
  ...
  try {
    ...
    const context = dom.canvas.getContext("webgpu") as GPUCanvasContext | null;
    if (!context) {
...
function syncRate(rawValue: string | number): void {
...
function startGame(): void {
  ...
  const totalNotes = calculateTotalNotes(state.rate);
...
function updateGame(now: number, dt: number): void {
  state.elapsed = now - state.startedAt;

  while (state.elapsed >= state.nextSpawnAt && state.nextSpawnAt <= SPAWN_CUTOFF + SPAWN_EPSILON) {
    spawnNote(state.nextSpawnAt);
    state.nextSpawnAt = FIRST_SPAWN_AT + state.spawned / state.rate;
  }
...
function spawnNote(spawnTime: number): void {
  const keyIndex = pickSpawnKeyIndex();
  ...
  const char = KEY_ORDER[keyIndex] ?? KEY_ORDER[0]!;
  const note = {
    id: state.nextNoteId,
    keyIndex,
    char,
    spawnTime,
    color: noteColorForKeyIndex(keyIndex),
...
function pickSpawnKeyIndex(): number {
  const start = Math.floor(Math.random() * KEY_ORDER.length);

  for (let offset = 0; offset < KEY_ORDER.length; offset += 1) {
    const candidate = (start + offset) % KEY_ORDER.length;
    if (!state.lastSpawned.includes(candidate)) {
      return candidate;
    }
  }
...
function calculateTotalNotes(rate: number): number {
  if (FIRST_SPAWN_AT > SPAWN_CUTOFF) {
    return 0;
  }

  return Math.floor((SPAWN_CUTOFF - FIRST_SPAWN_AT) * rate + SPAWN_EPSILON) + 1;
}
...
function calculateScore(): number {
  ...
  return Math.min(MAX_SCORE, Math.round((state.scoreUnits * MAX_SCORE) / state.totalNotes));
}

function noteColorForKeyIndex(keyIndex: number): Color {
  return NOTE_COLORS[keyIndex % NOTE_COLORS.length] ?? NOTE_COLORS[0]!;
}

function expireNotes(): void {
  for (let index = state.notes.length - 1; index >= 0; index -= 1) {
    const note = state.notes[index];
    if (note && state.elapsed - note.spawnTime >= NOTE_LIFETIME) {
      state.misses += 1;
      state.combo = 0;
...
function hitKey(key: string): void {
  expireNotes();

  const target = findHittableNote(key);
  if (!target) {
    state.combo = 0;
    state.wrong += 1;
...
function findHittableNote(key: string): Note | null {
  const limit = Math.min(HITTABLE_NOTE_LIMIT, state.notes.length);
...
function scoreForReaction(age: number): string {
  if (age <= 0.45) {
    return "SNAP";
  }
...
function createHitEffect(note: Note): void {
...
function notePosition(note: Note): NotePosition {
...
function flashKey(key: string, className: "hit" | "wrong"): void {
...
function drawIdlePreview(now: number): void {
  if (state.phase === "playing") {
    return;
  }
  ...
  for (let index = 0; index < 16; index += 1) {
    ...
    const color = noteColorForKeyIndex(keyIndex);
...
function drawStrikeZone(now: number): void {
...
function pushRect(x: number, y: number, width: number, height: number, color: readonly number[]): void {
  ...
  instanceData[offset + 4] = color[0] ?? 0;
  instanceData[offset + 5] = color[1] ?? 0;
  instanceData[offset + 6] = color[2] ?? 0;
  instanceData[offset + 7] = color[3] ?? 1;
  instanceCount += 1;
}
