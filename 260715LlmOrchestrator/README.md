...
`--search-provider auto`가 선택한 실제 공급자(`brave`, `searxng`, `ddgs`)도 설정 hash와 `run.json`에 기록됩니다. 재개 사이에 Brave 키 환경 변수나 SearXNG 설정이 바뀌어 실제 공급자가 달라지면 다른 작업으로 취급해 한 지식 맵에 출처 provenance가 섞이지 않게 합니다.
...
- `--base-url`, `--searxng-url`, `--site-url`에는 `user:password@host` 자격증명을 넣을 수 없습니다. 인증 비밀은 문서에 적힌 환경 변수 옵션으로만 전달하십시오.
