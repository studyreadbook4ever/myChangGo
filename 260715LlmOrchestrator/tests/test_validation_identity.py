...
from llm_orchestrator.prompts import GenerationContext, review_prompt
...
def test_model_authored_relative_link_is_rejected_before_site_rendering() -> None:
    ...
    assert any("상대 링크" in issue and "missing.md" in issue for issue in issues)
...
def test_semantic_duplicate_resolver_reuses_canonical_node_and_records_alias(config_factory) -> None:
    ...
    assert json.loads(dedupe_files[0].read_text(encoding="utf-8"))["relation"] == "same"
