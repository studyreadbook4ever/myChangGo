...
function captionReviewMessage(cue) {
...
function renderHeader() {
  ...
  const warnings = Array.isArray(project.ai?.warnings)
    ...
    : [];
  elements.caption_draft_warning.hidden = warnings.length === 0;
  if (warnings.length > 0) {
    ...
    elements.caption_draft_warning.textContent = reviewCount > 0
      ? `품질 검수 필요 ${reviewCount}건 · ${summary}. 표시된 컷 원음을 확인해 주세요.`
      : `키리누키 품질 하네스 자동 정리 ${warnings.length}건 · ${summary}`;
  } else {
    elements.caption_draft_warning.textContent = "";
  }
}
...
function renderCaptionDraftStatus() {
  elements.generate_captions.lastChild.textContent =
    " 활성 컷 전체 빈 타이밍 만들기";
...
function confirmCaptionDraftRun(enabledClips, { skippedClipCount = 0 } = {}) {
  const estimate = captionDraftRunEstimate(enabledClips);
  return window.confirm([
    skippedClipCount > 0
      ? "중단된 AudSeg 타이밍 초벌을 이어서 할까요?"
      : "AudSeg 빈 타이밍 초벌을 시작할까요?",
    "",
    ...(skippedClipCount > 0
...
async function generateCaptions() {
  if (!mediaFile) {
    showToast("자막 초벌을 만들려면 먼저 원본 영상을 연결해 주세요.", "error");
    return;
  }
  ...
  try {
    for (const clip of allEnabledClips) {
      audSegAudioFootprint(clipDurationMs(clip));
    }
  } catch (error) {
    showToast(error.message, "error", 0);
    return;
  ...
  const model = AUDSEG_DRAFT_MODEL;
  const runtime = audSegDraftRuntimeIdentity();
  const resumeCandidate = captionDraftResumePlan(
    allEnabledClips,
    project.ai?.captionCheckpoints,
    {
      resume: (
        ["running", "error", "canceled"].includes(project.ai?.status)
        && project.ai?.model === model
      ),
  ...
  if (!confirmCaptionDraftRun(enabledClips, {
  ...
  try {
    ...
    for (let index = 0; index < clips.length; index += 1) {
      ...
      controller.signal.throwIfAborted();
      const drafts = audSegBlankSubtitleDrafts(segmentation);
      const result = {
        provider: runtime.provider,
        model,
      ...
      if (generatedCueCount > MAX_CAPTION_DRAFT_CUES_PER_RUN) {
        throw new Error(
          `한 번에 만들 수 있는 AI 자막은 최대 ${MAX_CAPTION_DRAFT_CUES_PER_RUN.toLocaleString("ko-KR")}개입니다. 활성 컷을 나눠서 실행해 주세요.`
        );
      }
      ...
      project = {
        ...
        ai: {
          ...project.ai,
          provider: String(result.provider || "local-audseg"),
          model,
          resolvedModel: String(result.resolvedModel || result.model || model),
          lastRequestId: String(result.requestId || ""),
          captionCheckpoints: upsertCaptionDraftCheckpoint(
            project.ai?.captionCheckpoints,
            createCaptionDraftCheckpoint(clip, {
              requestId: result.requestId,
              pipelineFingerprint: runtime.fingerprint
            }),
            {
              maximum: allEnabledClips.length
            }
          ),
          status: "running",
      ...
      setAiProgress(
        base + span,
        `${index + 1}/${clips.length} · 빈 타이밍 저장 완료`
      );
    }
    project = {
      ...project,
    ...
    await saveProject(project);
    setAiProgress(1, "선택 구간 빈 타이밍 초안 완료");
    const draftLabel = `AudSeg ${AUDSEG_ENGINE_VERSION} 빈 타이밍`;
    showToast(
      generatedCueCount > 0
...
async function initialize() {
  bindActions();
  try {
    await chrome.storage.local.remove(RETIRED_CAPTION_SETTINGS_KEYS);
  } catch (error) {
    console.warn("사용하지 않는 자막 엔진 설정을 정리하지 못했습니다.", error);
  }
  renderCaptionDraftStatus();
  const {
    projectId,
    captureState,
    resumeSavedSession,
...
function normalizeLocalCaptionFirstPass(detail) {
  ...
  if (detail.cues.length > MAX_CAPTION_DRAFT_CUES_PER_RUN) {
    throw new TypeError(
      `한 번에 추가할 수 있는 로컬 자막은 최대 ${MAX_CAPTION_DRAFT_CUES_PER_RUN.toLocaleString("ko-KR")}개입니다.`
    );
  }
