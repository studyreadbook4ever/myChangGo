...
치지직 생방송이나 다시보기를 보며 사용할 구간을 표시하고, 같은 Extension의 전체화면 편집기에서 컷·구간별 음성·다중 한국어 자막을 검수해 영상을 내보내는 로컬 우선 도구입니다.
...
10. `영상 → 음성 → 자막` 타임라인에서 자막 텍스트·시간·위치·색상과 필요한 음성 구간의 음량·뮤트·페이드를 검수합니다. 자막 레인은 기본 2개이며 `+`로 늘릴 수 있고, 자막·음성 트랙을 우클릭해 현재 위치에 추가하거나 선택 블록을 삭제할 수 있습니다.
11. **영상 내보내기**에서 폴더를 한 번 고르면 선택 컷과 자막이 들어간 MP4 또는 WebM, 프로젝트 JSON, 자막이 있을 때의 SRT가 같은 폴더에 저장됩니다.
...
프롬프트는 `chzzk-kirinuki-edit-brief/v2`, 작업 폴더 manifest는 `chzzk-kirinuki-codex-job/v2`, 통합 편집기는 `chzzk-kirinuki-editor/v2` 규격을 사용합니다. Codex 작업폴더의 필수 결과 파일은 다음 다섯 개입니다.
...
- 사이드패널의 작은 캡처 상태는 `chrome.storage.local`, 편집 프로젝트·자막·파일 핸들은 IndexedDB에 자동 저장됩니다.
...
순수 프로젝트 모델에서는 방송 회차 분리, 사용자 확정 컷 변환, 시간축 매핑, v1→v2 마이그레이션, 다중 자막 레인·cue별 색상, 구간별 음성 자동화, AI 재실행 시 사람 수정 보존, 컷 재정렬과 SRT 출력을 단위 테스트합니다. 합성 영상 E2E는 파일 연결, 키보드·마우스 편집, IndexedDB 복구, 사이드패널 hot seed, 비영(非零) PTS A/V 컷 재정렬, WebCodecs 렌더, JSON/SRT와 최종 ZIP 로드를 확인합니다. 실제 한국어 음성으로 Whisper Tiny 전사와 단어 시각도 별도 확인했습니다.
