...
치지직 생방송이나 다시보기를 보며 사용할 구간을 표시하고, 같은 Extension의 전체화면 편집기에서 컷·한국어 자막·화면 위치를 검수해 영상을 내보내는 로컬 우선 도구입니다.

사용자가 찍은 시작·끝은 `authority: USER`인 확정 범위입니다. AI는 재미있는 구간을 대신 고르거나 경계를 자동으로 늘리지 않고, 선택된 범위의 한국어 자막 초안만 만듭니다. 텍스트, cue 시작·끝, 영상 위 자막 위치와 컷 경계는 사람이 직접 고칠 수 있습니다.
...
10. 자막 텍스트를 타이핑하고, 자막 블록 양끝과 영상 위 자막을 마우스로 끌어 시간·위치를 검수합니다.
...
프롬프트는 `chzzk-kirinuki-edit-brief/v2`, 작업 폴더 manifest는 `chzzk-kirinuki-codex-job/v2`, 통합 편집기는 `chzzk-kirinuki-editor/v1` 규격을 사용합니다. Codex 작업폴더의 필수 결과 파일은 다음 다섯 개입니다.
...
순수 프로젝트 모델에서는 방송 회차 분리, 사용자 확정 컷 변환, 시간축 매핑, 자막 텍스트·시간·위치 변경, AI 재실행 시 사람 수정 보존, 컷 재정렬과 SRT 출력을 단위 테스트합니다. 합성 영상 E2E는 파일 연결, 키보드·마우스 편집, IndexedDB 복구, 사이드패널 hot seed, 비영(非零) PTS A/V 컷 재정렬, WebCodecs 렌더, JSON/SRT와 최종 ZIP 로드를 확인합니다. 실제 한국어 음성으로 Whisper Tiny 전사와 단어 시각도 별도 확인했습니다.
