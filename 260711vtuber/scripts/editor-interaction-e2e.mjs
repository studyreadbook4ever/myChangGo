...
async function clickAndAcceptAudSegConfirmation(selector) {
  try {
...
async function main() {
  ...
  const aiProbeSetup = await executeSync(`
    ...
    globalThis.__kirinukiE2eAudSegWorker = {
      created: 0,
      posts: 0,
      terminated: 0,
      lastSampleRateHz: null
    };
    globalThis.Worker = class HangingAudSegWorker {
      constructor() {
        this.listeners = new Map();
        globalThis.__kirinukiE2eAudSegWorker.created += 1;
      }
      ...
      postMessage(message) {
        globalThis.__kirinukiE2eAudSegWorker.posts += 1;
        globalThis.__kirinukiE2eAudSegWorker.lastSampleRateHz =
          message?.sampleRateHz || null;
      }
      terminate() {
        globalThis.__kirinukiE2eAudSegWorker.terminated += 1;
      }
    };
    button.focus();
  ...
  assert(
    aiProbeSetup.activeId === "generate-captions"
      && aiProbeSetup.workerWrapped
      && aiProbeSetup.removedProviderUi,
    `AudSeg dialog probe 준비 실패: ${JSON.stringify(aiProbeSetup)}`
  );
  ...
  try {
    ...
    await waitUntil(async () => {
      const state = await executeSync(`
        return structuredClone(globalThis.__kirinukiE2eAudSegWorker || {});
      `);
      return state.posts === 1 ? state : false;
    }, "선택 컷 AudSeg Worker 분석 시작", { timeout: 20_000 });
    ...
    try {
      ...
      const actual = await executeSync(`
        ...
        return {
          ...
          buttonDisabled: button?.disabled,
          progressHidden: document.querySelector("#ai-progress")?.hidden,
          audsegWorker: globalThis.__kirinukiE2eAudSegWorker || null,
    ...
    aiWorkerProbe = await executeSync(`
      const result = structuredClone(globalThis.__kirinukiE2eAudSegWorker || {});
  ...
  assert(
    aiWorkerProbe?.created === 1 &&
      aiWorkerProbe?.posts === 1 &&
      aiWorkerProbe?.terminated === 1 &&
      aiWorkerProbe?.lastSampleRateHz === 16_000,
    `AudSeg Worker 요청·취소 계약이 지켜지지 않았습니다: ${JSON.stringify(aiWorkerProbe)}`
  );
  ...
  const audsegModeReady = await executeSync(`
    const description =
      document.querySelector("#caption-draft-description")?.textContent || "";
    return {
      description,
      status: document.querySelector("#caption-local-status")?.textContent || "",
      removedProviderUi: [
        "caption-model",
        "caption-agent-endpoint",
        "caption-agent-token",
  ...
  `);
  assert(
    audsegModeReady?.removedProviderUi === true &&
      audsegModeReady.description.includes("소리가 있는 구간만") &&
      audsegModeReady.description.includes("비어 있는 편집용 cue") &&
      audsegModeReady.description.includes("음성을 글로 바꾸지 않습니다") &&
      audsegModeReady.status.includes("AudSeg"),
    `AudSeg 고정 모드 안내가 올바르지 않습니다: ${JSON.stringify(audsegModeReady)}`
  );
  ...
  assert(
    aiSuccessSetup.wrapped &&
      aiSuccessSetup.removedProviderUi === true,
  ...
  try {
    ...
    const aiSuccessTimelineDom = await waitUntil(async () => {
      ...
      return (
        ...
        state.blankLabels.every((label) => label === "(빈 자막)") &&
    ...
    const aiSuccessListDom = await waitUntil(async () => {
      ...
      return (
        ...
        state.labels.every((label) => label === "(빈 자막)") &&
        state.titles.every((title) => title.includes("빈 오디오 타이밍"))
      ) ? state : false;
  ...
  await waitUntil(async () => {
    const state = await executeSync(`
      return {
        cueCount: document.querySelectorAll("#caption-tracks .cue-block").length,
        warningHidden: document.querySelector("#caption-draft-warning")?.hidden,
        warningText: document.querySelector("#caption-draft-warning")?.textContent || ""
      };
    `);
    return (
      state.cueCount === 0 &&
  ...
  const reopenedEditorState = await waitUntil(async () => {
    ...
    return (
      ...
      state.removedProviderUi === true
    ) ? state : false;
  }, "resume URL의 현재본 복원과 제거된 provider UI 비재생성");
