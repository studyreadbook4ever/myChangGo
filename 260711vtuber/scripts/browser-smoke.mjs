...
async function main() {
  ...
  const editor = await webdriver(baseUrl, "POST", `/session/${sessionId}/execute/sync`, {
    script: `
      ...
      return {
        ...
        draftDescription:
          document.getElementById("caption-draft-description")?.textContent || "",
        draftStatus:
          document.getElementById("caption-local-status")?.textContent || "",
  ...
  assert(
    editor.draftDescription.includes("소리가 있는 구간만")
      && editor.draftDescription.includes("비어 있는 편집용 cue")
      && editor.draftDescription.includes("음성을 글로 바꾸지 않습니다"),
    `AudSeg 비전사 설명이 보이지 않습니다: ${editor.draftDescription}`
  );
  assert(
    editor.draftStatus.includes("AudSeg"),
    `AudSeg 준비 상태가 보이지 않습니다: ${editor.draftStatus}`
  );
  ...
  assert(editor.removedProviderUi === true, "제거된 자막 provider UI가 다시 나타났습니다.");
  ...
  const runtime = await webdriver(baseUrl, "POST", `/session/${sessionId}/execute/async`, {
    script: `
      ...
      const timeout = setTimeout(() => done({ error: "AudSeg runtime timeout" }), 8_000);
      (async () => {
        await Promise.all([
        ...
        const stored = await chrome.storage.local.get(removedSettingKeys);
        clearTimeout(timeout);
          ...
          removedSettingsAbsent: removedSettingKeys.every(
  ...
  assert(runtime.localAgentPermission === false, "제거된 loopback host permission이 남아 있습니다.");
  assert(runtime.removedSettingsAbsent === true, "과거 provider 설정 키가 남아 있습니다.");
  assert(!runtime.cacheNames.includes("transformers-cache"), "이전 모델 캐시가 남아 있습니다.");
