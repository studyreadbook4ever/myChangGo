...
async function assertAbsent(relativePath) {
  ...
  throw new Error(`배포·라이선스 정책상 없어야 할 파일이 다시 생겼습니다: ${relativePath}`);
}
...
assert(
  ...
  "260711vtuber first-party package는 UNLICENSED여야 합니다."
);
await Promise.all([
  assertAbsent("LICENSE"),
  assertAbsent("extension/LICENSE"),
  assertAbsent("scripts/caption-gateway.mjs"),
  assertAbsent("scripts/local-caption-stack-core.mjs"),
  assertAbsent("scripts/local-caption-stack.mjs"),
  assertAbsent("scripts/create-synthetic-beta.py"),
  assertAbsent("scripts/solar-caption-gateway.mjs"),
  assertAbsent("src/caption-agent/caption-gateway-core.js"),
  assertAbsent("src/caption-agent/caption-quality-harness.js"),
  assertAbsent("src/caption-agent/editorial-context.js"),
  assertAbsent("src/caption-agent/protocol.js"),
  assertAbsent("src/caption-agent/solar-gateway-core.js"),
  assertAbsent("src/editor/caption-agent.js"),
  assertAbsent("tests/caption-agent-client.test.js"),
  assertAbsent("tests/caption-editorial-context.test.js"),
  assertAbsent("tests/caption-agent-protocol.test.js"),
  assertAbsent("tests/caption-gateway.test.js"),
  assertAbsent("tests/caption-quality-harness.test.js"),
  assertAbsent("tests/local-caption-stack.test.js"),
  assertAbsent("extension/knowledge/creator-policies/charon-universe-w.md")
]);
for (const [scriptName, command] of Object.entries(packageJson.scripts || {})) {
  assert(
    !/caption-(?:gateway|stack)|local-caption-stack|whisper|solar/iu.test(
      `${scriptName} ${command}`
    ),
    `삭제한 자막 companion 실행 명령이 package.json에 남아 있습니다: ${scriptName}`
  );
}
...
assert(
  packageLock.packages?.[""]?.license === "UNLICENSED",
  "package-lock first-party root license는 UNLICENSED여야 합니다."
);
assert(
...
assert(
  EXTENSION_PACKAGE_FILES.every((relativePath) => (
    !/whisper|caption-agent|asr-worker/iu.test(relativePath)
  )),
  "삭제한 Whisper/companion 파일이 Extension 배포 목록에 있습니다."
);
...
for (const forbiddenNotice of [
  ...
  assert(
    !notices.toLowerCase().includes(forbiddenNotice.toLowerCase()),
    `삭제한 Whisper runtime 고지가 남아 있습니다: ${forbiddenNotice}`
  );
}
