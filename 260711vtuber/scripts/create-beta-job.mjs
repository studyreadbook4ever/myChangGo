...
자세한 처리 경계는 **AUDSEG_TIMING.md**를 읽으세요. AudSeg가 선택 구간 안에서 빈 타이밍만 만든 뒤 사람이 원음을 듣고 글을 채우고 검수합니다.
`;
const toolsGuide = `# AudSeg 타이밍 베타

## 처리 경계

- 전체 원본이나 최종 영상을 외부 서비스에 보내지 않습니다.
- 편집기가 사용자 확정 컷마다 16kHz mono PCM을 현재 브라우저 탭에서 분석합니다.
- AudSeg는 모델 없이 빈 타이밍 cue만 만들며 글·화자·언어를 판정하지 않습니다.
- 빈 cue는 편집기에서 사람이 원음을 듣고 채운 뒤 기존 로컬 렌더러로 출력합니다.
- 인터넷 자막 서비스, 비밀 키와 별도 백그라운드 프로세스를 사용하지 않습니다.
...
await Promise.all([
  ...
  writeFile(
    ...
    `${codexAgents}\n\n## 이 합성 베타의 추가 지침\n\n- AUDSEG_TIMING.md를 읽고 AudSeg의 빈 타이밍 경계를 지킨다.\n- 실제 방송인 콘텐츠로 오인하지 않는다.\n`,
    "utf8"
  ),
  writeFile(path.join(jobDirectory, "START_HERE.md"), startHere, "utf8"),
  writeFile(path.join(jobDirectory, "AUDSEG_TIMING.md"), toolsGuide, "utf8"),
