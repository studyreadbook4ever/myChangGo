...
assert(!manifest.optional_host_permissions?.length, "원격 자막 host permission이 남아 있습니다.");
...
assert(
  ...
  "삭제한 로컬 companion loopback permission이 남아 있습니다."
);
...
try {
  ...
  errors.push("UNLICENSED first-party 프로젝트에 LICENSE 파일이 남아 있습니다.");
} catch (error) {
  if (error?.code !== "ENOENT") {
    errors.push(`first-party LICENSE 부재 확인 실패: ${error.message}`);
  }
}
...
assert(!panelScript.includes("writePolicyCacheFiles"), "재배포 허가 없는 정책 캐시 복사 로직이 남아 있습니다.");
...
assert(
  !serviceWorker.toLowerCase().includes("whisper"),
  "서비스 워커에 삭제한 Whisper runtime 흔적이 남아 있습니다."
);
for (const id of [
  "preview-video",
...
assert(editorScript.includes("audseg-local"), "편집기 번들에 AudSeg 로컬 모드가 없습니다.");
assert(editorScript.includes("segmentAudSegPcmInWorker"), "편집기 번들에 AudSeg Worker 요청 경로가 없습니다.");
assert(audsegWorkerScript.includes("segmentAudSegPcm"), "AudSeg Worker 번들에 활동 분석 경로가 없습니다.");
assert(audsegWorkerScript.includes("kirinuki-audseg-browser-result/v1"), "AudSeg Worker 결과 스키마가 없습니다.");
assert(
  editorScript.includes("allEnabledClips.length")
    && editorScript.includes("captionCheckpoints"),
  "AudSeg 전체 활성 컷 처리 또는 실패 지점 재개 체크포인트가 없습니다."
);
assert(
...
assert(editorHtml.includes("AudSeg"), "AudSeg 고정 자막 초벌 안내가 없습니다.");
assert(!editorHtml.includes('id="caption-model"'), "제거한 자막 초벌 엔진 선택 UI가 남아 있습니다.");
assert(!editorHtml.includes("API 키"), "자막 API 키 UI가 남아 있습니다.");
assert(editorHtml.includes("자막 하나는 최대 4초"), "편집기 UI에 4초 자막 원칙이 없습니다.");
for (const [label, content, forbiddenFragments] of [
  ...
  ["편집기 번들", editorScript, [
    ...
    "requestcaptionagent",
    "paircaptionagent",
    "bearer-process-memory",
    "/v1/captions",
    "/v1/session"
  ]],
  ["서비스 워커", serviceWorker, [
    "whisper",
    "127.0.0.1",
    "localhost",
    "/v1/captions",
  ...
  for (const forbiddenFragment of forbiddenFragments) {
    assert(
      !normalizedContent.includes(forbiddenFragment),
      `${label}에 삭제한 Whisper/loopback UI·runtime 흔적이 남아 있습니다: ${forbiddenFragment}`
    );
  }
}
...
assert(
  codexAgentGuide.includes("AudSeg"),
  "Codex 작업 규칙에 AudSeg 빈 cue 처리 범위가 없습니다."
);
assert(
  codexAgentGuide.includes("외부로 전송하지 않는다"),
  "Codex 작업 규칙에 원본·결과·프로젝트 외부 전송 금지가 없습니다."
);
assert(Array.isArray(policyIndex.policies) && policyIndex.policies.length === 5, "방송인 정책 인덱스가 5개 그룹을 포함하지 않습니다.");
assert(policyIndex.policies.every((policy) => !Object.hasOwn(policy, "cache")), "정책 인덱스에 재배포용 본문 캐시가 남아 있습니다.");
...
try {
  ...
  assert(arisaPrompt.includes("Redistribution: `LINK_ONLY`"), "아리사 작업 프롬프트에 링크 전용 재배포 규칙이 없습니다.");
  assert(!arisaPrompt.includes("policy-cache"), "아리사 작업 프롬프트에 정책 캐시 경로가 남아 있습니다.");
  ...
  assert(!Object.hasOwn(smokeManifest.inputs, "creatorPolicyCache"), "작업 manifest에 정책 캐시 입력이 남아 있습니다.");
