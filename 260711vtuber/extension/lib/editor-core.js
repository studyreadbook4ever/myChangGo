...
export function normalizeEditorProject(raw) {
  ...
  const rawAi = raw.ai || {};
  const supportedAiMetadata = (
    (
      rawAi.provider === defaults.ai.provider
      && rawAi.model === defaults.ai.model
    )
    || rawAi.provider === "codex-local-first-pass"
  );
  const ai = {
    ...defaults.ai,
    ...(supportedAiMetadata ? rawAi : {}),
...
export function isAudSegBlankTimingCue(cue) {
  return Boolean(
    cue?.origin === "ai"
...
export function subtitleCueNeedsReview(cue) {
  return Boolean(
    cue?.origin === "ai"
...
export function replaceAiBlankTimingDraft(project, clipId, drafts = []) {
  const rangeSignature = (cue) => (
    (Array.isArray(cue?.remoteMeta?.qualityCodes)
      ? cue.remoteMeta.qualityCodes
      : [])
      .find((code) => /^AUDSEG_RANGE_\d+_\d+$/u.test(code))
    || null
  );
