...
그다음 편집기에서 Upstage API 키를 **현재 탭에만** 붙여 넣고 Solar Mini 또는 Solar Pro 3를 선택한 뒤 **활성 컷 전체 Solar 자막 초안 만들기**를 누릅니다. 로컬 Whisper 주소, STT 키, 세션 토큰은 입력하지 않습니다.

```text
활성 컷의 16kHz WAV
  → 이 기기의 whisper.cpp
  → canonical timed units
  → 컷당 정확히 1회의 Upstage Solar Mini/Pro 3 호출
  → 추가 유료 호출 0회의 로컬 kr-vtuber-clean-v1 품질 하네스
  → 한 줄·하단 고정 편집기 자막 cue
```
...
- 권한이 있는 로컬 원본 파일 편집
- Linux의 자동 로컬 Whisper 설치·실행
- CPU 기본, NVIDIA + CUDA compiler가 모두 준비된 Linux에서 CUDA build
- Solar Mini와 Solar Pro 3
- 외부 timed STT의 고급 호환 경로
...
- `npm run caption-stack:doctor`: Node, RAM, CPU, NVIDIA/CUDA, 빌드 도구, 포트, 기존 설치를 읽기 전용 점검합니다. 유료 API를 호출하지 않습니다.
...
| 프로필 | 모델 | 사용 시점 |
|---|---|---|
| `auto` | 보통 `small-q5_1`; RAM 6GiB 미만은 자동 `light` | 대부분의 첫 설치 |
| `light` | `base-q5_1` | 저사양 CPU, 메모리 부족, 빠른 초벌 |
...
한국어 작업이므로 `.en` 모델을 사용하지 않습니다. 기본은 CPU이며, Linux에서 `nvidia-smi`와 `nvcc`를 모두 확인한 경우에만 `auto` backend가 CUDA를 선택합니다.
...
1. **Solar 모델**을 고릅니다.
   - `Solar Mini`: 저렴하고 빠른 첫 초벌
   - `Solar Pro 3`: 문맥·표현 품질 우선
2. Upstage API 키를 **Solar API 키 · 현재 탭에서만**에 붙여 넣습니다.
3. 상태가 `로컬 STT 준비됨`인지 확인합니다.
4. **활성 컷 전체 Solar 자막 초안 만들기**를 누릅니다.
5. 실행 전 창에서 활성 컷 수, 총 길이와 활성 컷 수와 같은 최대 Solar 요청 수를 확인합니다.
6. 계속할 때만 확인합니다.
...
- `whisper-server`: 고정 모델을 이용한 로컬 전사
- `solar-caption-gateway.mjs`: 인증, 요청 상한, STT→Solar→로컬 하네스 변환
...
`ADVANCED` 기존 외부 timed STT 호환도 유지합니다. 편집기의 고급 설정에서 endpoint, model, key를 입력할 수 있습니다.
...
| 매우 느림 | `--profile light --backend cpu`로 setup 후 재시작 |
| 메모리 부족 | `light`, 다른 무거운 앱 종료, 활성 컷을 더 작은 묶음으로 실행 |
| 짧은 감탄사 누락 | 원음을 듣고 직접 cue 추가; 필요하면 `quality`로 해당 작업 재시도 |
| Solar 401/429 | 현재 탭의 키·계정 상태·rate limit 확인; 전체 버튼을 연속 클릭하지 않기 |
| `SOLAR_RESPONSE_FORMAT_UNSUPPORTED` 또는 빈 결과 | 자동 유료 형식 폴백은 하지 않음; 모델·설정을 확인한 뒤 사용자가 재실행 여부 결정 |
| 중간 실패 | 같은 모델·같은 컷 범위·같은 하네스 지문으로 버튼을 다시 눌러 체크포인트 재개 |
...
7. 외부 네트워크 테스트는 opt-in으로 두고 기본 테스트는 fake STT/Solar만 사용합니다.
...
- 16kHz WAV가 Upstage 요청에 포함되지 않음
- 로컬 mode가 STT key 없이 준비 완료
- 외부 remote endpoint는 HTTPS와 명시적 STT key 요구
