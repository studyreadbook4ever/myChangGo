...
Solar Pro 3와 Mini는 텍스트 모델이며 오디오나 영상을 직접 전사하지 못합니다. 따라서 먼저 로컬 whisper.cpp가 음성을 타임코드가 있는 텍스트로 바꿉니다. 화면 분석에는 영상 대신 로컬에서 계산한 숫자형 방해도만 사용합니다. Upstage에는 timed transcript·이름·메모·숫자형 위치 요약과 상한이 고정된 용어·화자 별칭·사람 검수 문체 예문을 보냅니다. 원본 전체, WAV, 대표 프레임 픽셀, 선택하지 않은 구간, 이미지 에셋과 최종 렌더는 보내지 않습니다.
...
- 자막 생성에는 Upstage 네트워크와 비용·rate limit이 적용됩니다. 기본 경로에서 선택 컷 오디오는 로컬에 남고, Upstage에는 timed transcript·이름·메모·숫자형 위치 요약과 상한이 고정된 용어·화자 별칭·사람 검수 문체 예문이 가므로 Upstage 데이터 정책을 확인하세요.
