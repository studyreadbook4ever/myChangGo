...
타임라인 순서는 `영상 → 음성 → 자막`입니다. 음성은 원본의 주 오디오 트랙을 나타내는 고정 1개 레인이며 별도 음원 분리를 하지 않습니다.
...
- `.kirinuki.json`: 컷, 순서, 다중 자막 레인·색상, 음성 설정 구간과 방송 메타데이터를 담은 `chzzk-kirinuki-editor/v2` 프로젝트 스냅샷
...
- 현재 편집기는 컷 순서·경계, 다중 자막 레인과 기본 음성 자동화에 집중합니다. 전환, 다중 오디오 트랙, 음원 분리, 고급 색보정과 플러그인 믹싱은 제공하지 않습니다.
