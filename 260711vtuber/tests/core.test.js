...
test("아티스트 정책 링크와 필수 안전 게이트가 기본 정책에 등록되어 있다", async () => {
  ...
  assert.match(policy, /LINK_ONLY \/ UNVERIFIED_AT_RUN/);
});
...
test("아리사를 카론유니버스W 공식 링크에 정확히 매칭하고 정책 본문은 재배포하지 않는다", async () => {
  const [indexText, basePolicy] = await Promise.all([
  ...
  assert.equal(Object.hasOwn(index.policies.find((policy) => policy.id === "charon-universe-w"), "cache"), false);
  ...
  assert.match(compiled, /Redistribution: `LINK_ONLY`/);
  assert.doesNotMatch(compiled, /policy-cache/);
  ...
  assert.equal(Object.hasOwn(metadata.creatorPolicyResolution[0], "cache"), false);
  assert.match(prompt, /https:\/\/cafe\.naver\.com\/vkpopstar\/1174/);
  assert.doesNotMatch(prompt, /policy-cache/);
});
