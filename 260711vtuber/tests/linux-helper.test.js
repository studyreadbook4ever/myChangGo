...
test("CLI는 AudSeg 전용 명령과 옵션만 파싱한다", () => {
  assert.deepEqual(
    parseLinuxHelperArgs([
      "setup",
  ...
  assert.equal(parseLinuxHelperArgs(["start"]).command, "start");
  assert.throws(
    () => parseLinuxHelperArgs(["setup", "--api-key", "secret"]),
    /비밀 값/u
  );
  assert.throws(
    () => parseLinuxHelperArgs(["setup", "--mode", "remote"]),
    /지원하지 않는 옵션/u
  );
});
...
test("XDG 경로는 저장소 밖의 안정적인 사용자 profile을 고른다", () => {
  ...
  assert.equal(
    paths.configPath,
    "/tmp/config root/kirinuki-studio/helper.json"
  );
  assert.equal(
    paths.stateRoot,
    "/tmp/state root/kirinuki-studio"
  );
});

test("도움말은 AudSeg 전용 사용법과 안전 경계를 노출한다", () => {
  const text = helpText();
  for (const command of ["setup", "doctor", "open", "start", "status"]) {
    assert.match(text, new RegExp(`\\b${command}\\b`, "u"));
  }
  assert.match(text, /AudSeg/u);
  assert.match(text, /모델·서버·API 키·백그라운드 서비스가 필요하지 않습니다/u);
  assert.match(text, /브라우저 강제 종료/u);
  assert.doesNotMatch(text, /curl\s*\|\s*(?:ba)?sh/iu);
});

test("fresh Linux dry-run은 외부 변경 없이 setup과 open 명령을 보여준다", async (t) => {
...
test("누락된 fresh Linux 의존성은 변경 없이 설치 안내로 실패한다", async () => {
  const result = await runNode(["doctor"], {
    env: {
      ...process.env,
      PATH: ""
    }
  ...
  assert.match(result.stdout, /자동 설치하지 않습니다/u);
});

test("상태와 stop 호환 명령은 백그라운드 서비스를 요구하지 않는다", async (t) => {
  const tempRoot = await mkdtemp(
  ...
  const status = await runNode(["status", "--json"], { env });
  assert.equal(status.code, 0, status.stderr);
  const parsed = JSON.parse(status.stdout);
  assert.equal(parsed.mode, "audseg-local");
  assert.equal(parsed.services.required, false);

  const stopped = await runNode(["stop"], { env });
  assert.equal(stopped.code, 0, stopped.stderr);
  assert.match(stopped.stdout, /중지할 백그라운드 서비스가 없습니다/u);
});

test("셸 진입점은 ZIP의 0644 권한에서도 setup을 열고 실행권한을 복원한다", async (t) => {
