...
test("모든 child 환경은 API 비밀을 제거하고 loopback proxy 우회를 강제한다", () => {
  const sanitized = withoutCaptionSecrets({
    PATH: "/usr/bin",
    UPSTAGE_API_KEY: "upstage-secret",
...
test("systemd-user unit은 자동 페어링·로컬 STT·exact Origin만 환경에 넣는다", () => {
  ...
  assert.doesNotMatch(
    unit,
    /UPSTAGE_API_KEY|KIRINUKI_AGENT_TOKEN|KIRINUKI_STT_API_KEY/u
  );
  assert.throws(
    () => renderSystemdUserUnit({
      nodePath: "/usr/bin/node",
...
test("CLI source는 gateway를 오케스트레이션하되 키를 env·argv에 주입하지 않는다", async () => {
  ...
  assert.match(source, /solar-caption-gateway\.mjs/u);
  ...
  assert.doesNotMatch(source, /KIRINUKI_STT_API_KEY\s*:/u);
  assert.doesNotMatch(source, /UPSTAGE_API_KEY\s*:/u);
