...
test("AI 자막 체크포인트는 현재 컷·범위·지원 모델만 제한 개수로 복원한다", () => {
  ...
  const currentPipeline = normalizeAiCaptionCheckpoints([{
    ...valid,
    pipelineFingerprint: "audseg-pipeline-v1"
  }], clips);
  assert.equal(
...
test("과거 실행 메타데이터는 폐기하되 기존 자막 cue는 저장 왕복에서 보존한다", () => {
  ...
  const normalized = normalizeEditorProject({
    ...
    ai: {
      ...current.ai,
      provider: "retired-local-provider",
      model: "retired-model",
      resolvedModel: "retired-runtime",
      status: "done",
      progress: 1
    }
  });
  assert.equal(normalized.ai.provider, "local-audseg");
  assert.equal(normalized.ai.model, "audseg-local");
  assert.equal("resolvedModel" in normalized.ai, false);
  assert.equal(normalized.ai.status, "idle");
  assert.equal(normalized.ai.progress, 0);
  assert.equal(normalized.subtitles.length, 1);
  assert.equal(normalized.subtitles[0].text, "기존 자막");
});
