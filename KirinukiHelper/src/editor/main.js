...
function renderCaptionModeControls() {
  ...
  const whisperRuntime = captionAgentRuntime?.provider === "local-whispercpp"
    ? captionAgentRuntime
    : null;
  const actualStt = whisperRuntime?.sttModel || "Tiny 기본";
  ...
  if (audseg) {
    ...
    elements.caption_local_status.dataset.state === "ready"
    && whisperRuntime
  ) {
    setCaptionLocalStatus(
      `로컬 Whisper ${actualStt} 준비됨`,
      "ready"
    );
  } else {
    setCaptionLocalStatus(
      "로컬 Whisper companion을 자동으로 찾습니다",
      "idle"
    );
  }
}
...
function confirmCaptionAgentRun(enabledClips, { skippedClipCount = 0 } = {}) {
  ...
  return window.confirm([
    skippedClipCount > 0
      ...
      : `${audseg ? "AudSeg 빈 타이밍" : "Whisper Tiny 로컬 자막"} 초벌을 시작할까요?`,
    "",
    ...(skippedClipCount > 0
    ...
    ...(audseg
      ...
      : [
        `로컬 Whisper ${captionAgentRuntime?.sttModel || "Tiny 기본"} · 유료 API 호출 없음`,
...
async function testCaptionAgentConnection() {
  ...
  try {
    ...
    const ready = result.configured?.ready !== false;
...
async function generateCaptions() {
  ...
  try {
    ...
    if (!audsegMode) {
      elements.caption_advanced_settings.open = true;
      setCaptionLocalStatus(
        "로컬 Whisper 연결 필요 · 세부설정에서 실행 상태를 확인하세요",
        "error"
      );
    }
  ...
  try {
    ...
    if (
      !canceled
      && /(?:STT|전사|companion|에이전트)/iu.test(error.message)
    ) {
      elements.caption_advanced_settings.open = true;
    }
    project = {
...
async function initialize() {
  ...
  renderCaptionModeControls();
  if (
    captionAgentSettings.model !== AUDSEG_DRAFT_MODEL
    && isLoopbackCaptionAgentEndpoint(captionAgentSettings.endpoint)
  ) {
    const startupCaptionConfig = readCaptionAgentConfig();
    void ensureLocalCaptionSession(startupCaptionConfig)
      .catch(() => {
        if (!captionAgentSelectionStillCurrent(startupCaptionConfig)) {
          return;
        }
        setCaptionLocalStatus(
          "로컬 Whisper가 꺼져 있습니다 · 세부설정의 실행 안내를 확인하세요",
          "offline"
        );
      });
  }
