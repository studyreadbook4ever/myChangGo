...
test("프로토콜은 로컬 Whisper 전용 모델·provider 계약을 공개한다", () => {
  assert.equal(
  ...
  assert.equal(
    Object.hasOwn(CAPTION_AGENT_REQUEST_JSON_SCHEMA.properties, "visual"),
    false
  );
  assert.equal(
    Object.hasOwn(
      CAPTION_AGENT_REQUEST_JSON_SCHEMA.properties.policy.properties,
      "placement"
    ),
    false
  );
...
test("caption-agent 요청은 스키마·한국어·클립 길이·base64를 엄격히 검증한다", () => {
  ...
  assert.throws(
    () => validateCaptionAgentRequest(protocolRequest({ model: "remote-model" })),
...
test("편집기가 만드는 실제 요청과 companion 계약이 같은 버전·필드를 사용한다", () => {
  ...
  assert.equal(normalized.model, "whisper-tiny");
  assert.equal(Object.hasOwn(normalized, "visualPlacement"), false);
...
test("[불명확] 자막은 false를 반환해도 사람 검수를 강제한다", () => {
  const result = normalizeCaptionCuesDetailed([{
    startMs: 0,
    endMs: 900,
    text: "[불명확]",
...
test("원시 cue와 응답 warning 개수 상한을 처리 전에 강제한다", () => {
  assert.throws(
    () => normalizeCaptionCuesDetailed(
      Array.from({ length: MAX_CAPTION_CUES + 1 }, () => ({})),
      { clipDurationMs: 1_000 }
    ),
    (error) => error?.code === "CAPTION_CUE_LIMIT_EXCEEDED"
  );
...
test("로컬 Whisper 이외의 응답 provider는 거절한다", () => {
  const request = validateCaptionAgentRequest(protocolRequest());
  assert.throws(
    () => createCaptionAgentResponse({
      request,
    ...
    (error) => error?.code === "INVALID_RESPONSE_PROVIDER"
  );
});
